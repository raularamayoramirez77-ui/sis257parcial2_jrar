...
const nivelesFiltrados = computed(() => {
  return nivelesAcademicos.value.filter((nivel) =>
...
<template>
  <div>
    ...
    <table>
      ...
      <tbody>
        <tr v-for="(nivel, index) in nivelesFiltrados" :key="nivel.id">
          <td>{{ index + 1 }}</td>
          <td>{{ nivel.nombre }}</td>
          <td>{{ nivel.descripcion || '-' }}</td>
          <td class="actions">
            <button class="btn-icon btn-edit" @click="emitirEdicion(nivel)" title="Editar">
              ✏️
            </button>
            <button class="btn-icon btn-delete" @click="mostrarEliminarConfirm(nivel)" title="Eliminar">
              🗑️
            </button>
          </td>
        </tr>
        <tr v-if="nivelesFiltrados.length === 0">
